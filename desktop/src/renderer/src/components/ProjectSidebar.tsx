...
function SessionRow({
	...
	return (
		<div
			className={`lb-session-row ${selected ? "is-selected" : ""} ${disabled ? "is-disabled" : ""}`}
		>
			<button
				type="button"
				className="lb-session-select"
				onClick={onSelect}
				disabled={disabled}
				title={session.title}
			...
			>
				<div className="lb-session-mainline">
					<div className="lb-session-text">
						<p className="lb-session-title">{session.title}</p>
						{subtitle ? (
							<p className="lb-session-subtitle">{subtitle}</p>
						) : null}
						<p className="lb-session-meta">
							{ui.isStreaming ? (
								<span className="lb-state-dot is-streaming" />
							) : null}
	...
	return (
		<aside className="lb-sidebar">
			<div className="lb-sidebar-primary">
				...
				<button
					type="button"
					className="lb-primary-item is-visual-only"
					tabIndex={-1}
				>
					<span className="lb-leading-icon">◷</span>
						...
						<div className="lb-session-stack">
							{pinnedSessions.map((entry) => (
								<SessionRow
									...
									subtitle={
										entry.session.availability === "available"
											? entry.projectName
											: `Unavailable${entry.session.warningReason ? ` · ${entry.session.warningReason}` : ""}`
									}
									pinned
									disabled={entry.session.availability !== "available"}
									onSelect={() =>
										onSelectSession({
											sessionId: entry.session.id,
											projectId: entry.projectId,
											availability: entry.session.availability,
											source: entry.session.source,
											warningReason: entry.session.warningReason,
										})
									}
							...
							return (
								<section className="lb-project-group" key={project.id}>
									<div className="lb-project-header-row">
										...
										>
											<span className="lb-project-disclosure">
												{collapsed ? "▸" : "▾"}
											</span>
									...
									{!collapsed ? (
										<div className="lb-session-stack">
											{sessions.map((session) => {
												...
												const disabled = session.availability !== "available";
												return (
													<SessionRow
														key={session.id}
														session={session}
														...
														subtitle={
															disabled
																? session.warningReason
																	? `Unavailable · ${session.warningReason}`
																	: "Unavailable"
																: undefined
														}
														onSelect={() =>
